...
<c:if test="${ empty servicos }">
     				<center><font color="red">Nenhum serviço cadastrado até o momento!</font>  </center>
     			</c:if>
		...
		<h2 align="center">Lista de Serviços</h2>
  		<table class="table table-bordered table-hover">
    		...
    		<c:forEach var="s" items="${servicos}">
                <tr>
                  <td><c:out value="${s.descricao}"></c:out></td>
                  <td><c:out value="${s.valor}"></c:out></td>
                  <td><c:out value="${s.disponivel eq true ? 'Disponível' : 'Indisponível'}"></c:out></td>
                  <td><a href="buscaservico?id=${s.codigo}" >Alterar</a> </td>                </tr>
              </tbody> </c:forEach>
