...
<div class="col-xs-12 col-sm-9">
	...
	<div class="jumbotron"><c:if test="${not empty sucess}">
			 <script> 
       			//<!-- alert ("${sucess}");--!> 
       			 msg('${sucess}');  // esta é uma chamada para a função que deve ser definido na seção <head> ou outro arquivo JS. 
   			 </script>
    	...
    	<c:if test="${not empty loginUsuario }">
				<c:if test="${not empty errosServicos}">
					<ul>
						<c:forEach var="e" items="${errosServicos}">
				...
				<center>
					...
					<br>
					<form action="alteraservicoagenda" method="post">
					<!-- ANIMAIS DISPONIVEIS --><table>
						<tr>
							<td>Animal: <select name="animal">
									<c:if test="${not empty agendaservico.usuario.animais}">
										<c:forEach var="a" items="${agendaservico.usuario.animais}">
											<option value="${a.codigo}" ${a.codigo eq agendaservico.animal.codigo ? 'selected' : ' '}>
												<c:out value="${a.nome}"></c:out>
											</option>
										</c:forEach>
									</c:if>
							</select> <a href='cadastroAnimalserv.jsp'><span><img
										src='imagens/mais.png' width='18' /></span></a></td>
						</tr><br>
						<tr>
						<td><br>Serviços:</td></tr><tr>			
						<td> <input type="checkbox" name ="descricao" value="${agendaservico.servico.codigo}" checked="checked" readonly="true">
							 <c:out value="${agendaservico.servico.descricao}">
							 </c:out> <c:out value="${agendaservico.servico.valor}"></c:out></td></tr>
						</table>
<p>
						<table>
							<tr>
								<td>Data:</td>
								<td><input type="date" name="dataCadastro"	value="${agendaservico.dataFormAmericano()}" placeholder="05/02/2015" /></td>
							</tr>
							<td><p></td>
							<tr>
								...
								<td><select name="horario">
										<option value=""></option>
										<option value="8" ${agendaservico.hora eq '8' ? 'selected' : ' '}>8:00</option>
									<option value="10" ${agendaservico.hora eq '10' ? 'selected' :' ' }>10:00</option>
									<option value="12" ${agendaservico.hora eq '12' ? 'selected' : ' '}>12:00</option>
									<option value="14" ${agendaservico.hora eq '14' ? 'selected' : ' '}>14:00</option>
									<option value="16" ${agendaservico.hora eq '16' ? 'selected' : ' '}>16:00</option>
								</select></td>
							</tr>
							<tr><td><input type="hidden" name="codigo" value="${agendaservico.codigo}"/>
						</table>
				 
						<input type="hidden" name="disponivel" value="1">
						
						<br> <input type="checkbox" name="taxiDog" value="1" checked=${agenda.buscar eq true ? 'checked' :' ' }> Buscar
						Pet em Casa (TaxiDog)<br> <br> <input type="submit"
							value="Atualizar"><br>
							
					</form>

					<br>*Os serviços fornecidos são somente para cães e gatos.<br>
					*O atendimento é realizado por ordem de chegada.
			</c:if>
         <c:if test="${empty loginUsuario }">
				<center> <font color="red"> <h2>Você precisa estar logado para solicitar um servico</h2></font> </center>
			</c:if>	</div><!-- Fecha o jumbotron -->
