...
<div class="col-xs-12 col-sm-9">
	...
	<c:if test="${not empty erroAtualizacao}">
		...
		<c:if test="${not empty agendaServicos }">
		
		<div class="table-responsive">
		<h2 align="center">Lista de Serviços </h2>
  		<table class="table table-bordered table-hover">
    		...
    		<tbody>
    		<c:forEach var="a" items="${agendaServicos}">
                <tr>
                		<td class="lalign"><c:out value="${a.trataString()}"></c:out></td><td><c:out value="${a.hora}:00"></c:out></td>

														<td><c:out value="${a.usuario.cpf eq '00000000000' ? 'Sem Dono' : a.usuario.primeiroNome}"></c:out></td>
														
														<td><c:out value="${a.servico.descricao}"></c:out></td>
														...
														<c:if test="${a.usuario.cpf != '00000000000'}">
															<td><a href="alteraservicoadm?codigoServico=${a.codigo}">Alterar</a></td>
														</c:if>
														<c:if test="${a.usuario.cpf == '00000000000' }">
															<td>Alterar</td>
														</c:if>
														<td><a href="deleteservicoadm?codigoServico=${a.codigo}">Cancelar</a></td>

</tr></c:forEach>
              </tbody> 
             
  		</table>
  		 <input type="hidden" name="atualizar" size="55" value="1" maxlength="14">
	</div>
		</c:if>
		<c:if test="${empty agendaConsulta}">
							
								<center> <font color="red"> <h2>Não tem nenhuma solicitacao de servico agendada<br><br><br><br></h2></font></center>
							</c:if>
